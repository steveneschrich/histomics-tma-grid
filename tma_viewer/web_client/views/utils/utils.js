...
const TMA_METADATA_FILE_NAME = "tma-structure.json";
const STAIN_METADATA_FILE_NAME = "stain.json";
...
const getTissuePositionInName = tissueName => {
  ...
  return parseInt(tissuePositionString.charAt(tissuePositionString.length - 4));
};
...
const groupByPositionInName = array => {
  ...
  unique.forEach(item => {
    result.push(
      array.filter(e => getTissuePositionInName(e.name) === parseInt(item)),
    );
  });
  return result;
};
...
export const getTMAData = (folderId, hightlightId, imageSize = 150) => {
  ...
  return getParentFolderId(folderId)
    ...
    .then(itemsId => {
      return Promise.all([
        getTMAStructureJSONContent(itemsId.TMAMetadataJSONStructureItemId),
      ...
      if (
        ...
        events.trigger("query:tma-table-data", {
          ...
          error: `Rows count: ${TMAMetadataJSONStructure.design.num_rows}
                            Columns count: ${TMAMetadataJSONStructure.design
                              .num_cols}
                            Core Lenght: ${TMAMetadataJSONStructure.design.cores
                              .length}`,
        });
        return;
      }
        ...
        Promise.all(
          folderItems.map(item => {
            return {
              ...
              isHightlighted:
                hightlightId !== undefined && hightlightId === item._id,
            };
          }),
        ).then(tmaItems => {
          let resultingMatrix = Array(
            ...
            .fill()
            .map(() =>
              Array(parseInt(TMAMetadataJSONStructure.design.num_cols)).fill(),
            );

          const designTMACores = TMAMetadataJSONStructure.design.cores;
          ...
          tmaItems.forEach(item => {
            ...
            if (selectedCore) {
              const designCore = designTMACores.find(
                core => core.study_core_id === selectedCore.study_core_id,
              );

              const coreAnnotation = designTMACores.find(
                core => core.study_core_id === designCore.study_core_id,
              );

              item.study_core_id = designCore.study_core_id;
              item.tma_number = designCore.tma_number;
              item.core_id = designCore.core_id;
              item.core_label = designCore.core_label;
              item.row_index = designCore.row_index;
              item.row_label = designCore.row_label;
          ...
          const currentLayout = STAINMetadataJSONStructure.visualization
          ...
          const result = {
            ...
            name: TMAMetadataJSONStructure.name,
